            ...
            <MorphingDialogDescription className="flex-1 min-h-0 overflow-hidden">
                <GroupEditor
                    ...
                    initial={{
                        ...
                        max_concurrency: group.max_concurrency ?? 0,
...
export function GroupCard({ group }: { group: Group }) {
    ...
    const handleSubmitEdit = useCallback((values: GroupEditorValues, onDone?: () => void) => {
        ...
        const nextMaxConcurrency = Math.max(0, Math.floor(values.max_concurrency ?? 0));
        ...
        if (nextMaxConcurrency !== (group.max_concurrency ?? 0)) payload.max_concurrency = nextMaxConcurrency;
    ...
    }, [group.first_token_time_out, group.session_keep_time, group.max_concurrency, group.id, group.items, group.match_regex, group.mode, group.name, onSuccess, onError, updateGroup]);
